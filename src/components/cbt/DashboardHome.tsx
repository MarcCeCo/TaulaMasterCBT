...
// Dashboard d'inici — stats pures, sense Quick Access (el sidebar ja hi és).
...
import {
  ...
  ListChecks, Package, Table2, WifiOff, RefreshCw,
} from "lucide-react";
...
function StatCard({
  ...
  return (
    <Card className={`p-5 flex items-center gap-4 border-0 shadow-sm transition-shadow hover:shadow-md ${highlight ? "bg-amber-50 border border-amber-200" : "bg-white"}`}>
      <div className={`h-12 w-12 rounded-xl flex items-center justify-center shrink-0 ${iconBg} ${iconColor}`}>
        {icon}
      </div>
      <div className="min-w-0 flex-1">
        <div className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold leading-none mb-1">{label}</div>
        <div className={`text-2xl font-bold leading-tight ${highlight ? "text-amber-600" : "text-slate-800"}`}>{value}</div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Visió general de l'estat de la Taula Master</p>
        </div>
      ...
      {error && !loading && (
        <Card className="p-4 border border-amber-200 bg-amber-50 flex items-center gap-3">
          ...
          <Button size="sm" variant="outline" className="border-amber-300 text-amber-800 hover:bg-amber-100 shrink-0" onClick={retry}>
            <RefreshCw className="h-3.5 w-3.5 mr-1.5" /> Reintenta
          </Button>
        </Card>
      )}
      ...
      {canSeeView("equips") && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {loading ? (
            ...
            <>
              <StatCard
                icon={<Package className="h-6 w-6" />} iconBg="bg-violet-100" iconColor="text-violet-600"
                label="Equips totals" value={stats.totalEquips}
                sub={<><span className="text-emerald-500 font-semibold">{stats.equipWithTable}</span> amb taula definida</>}
              />
              <StatCard
                icon={<Database className="h-6 w-6" />} iconBg="bg-[#0099A8]/10" iconColor="text-[#0099A8]"
                label="Camps al diccionari" value={stats.total}
                sub={`${stats.usagePercent}% en ús`}
              />
              <StatCard
                icon={<ListChecks className="h-6 w-6" />} iconBg="bg-emerald-100" iconColor="text-emerald-600"
                label="Camps en ús" value={stats.usedCount}
                sub={`De ${stats.total} disponibles`}
              />
              <StatCard
                icon={<Layers className="h-6 w-6" />} iconBg="bg-blue-100" iconColor="text-blue-600"
                label="Grups de camps" value={stats.fieldGroups}
                sub="Agrupacions del diccionari"
              />
              <StatCard
                icon={<Table2 className="h-6 w-6" />} iconBg="bg-sky-100" iconColor="text-sky-600"
                label="Amb taula" value={stats.equipWithTable}
              ...
              <StatCard
                ...
                label="Refs. trencades" value={stats.orphanCount}
