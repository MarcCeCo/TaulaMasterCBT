...
import { Database, Layers, ListChecks, Package, Table2, AlertTriangle } from "lucide-react";
...
export function TaulaMasterMain() {
  ...
  const stats = useMemo(() => {
    ...
    const groups = Array.from(new Set(fields.map((f) => f.group).filter(Boolean) as string[]));
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <Card className="p-4 flex items-center gap-3 border-0 shadow-sm bg-white">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex flex-wrap gap-1 mt-0.5">
                ...
                {stats.groups.length > 2 && <Badge variant="outline" className="text-[10px] px-1.5 py-0">+{stats.groups.length - 2}</Badge>}
              </div>
            </div>
          </Card>
          ...
          <Card className="p-4 flex items-center gap-3 border-0 shadow-sm bg-white">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground"><span className="text-emerald-600 font-semibold">{stats.equipWithTable}</span> amb taula</div>
            </div>
          </Card>
